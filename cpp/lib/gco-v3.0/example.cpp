...
struct ForDataFn{
	int numLab;
	GCoptimization::EnergyTermType *data;
};


GCoptimization::EnergyTermType smoothFn(int p1, int p2, int l1, int l2)
{
	if ( (l1-l2)*(l1-l2) <= 4 ) return((l1-l2)*(l1-l2));
	else return(4);
}

GCoptimization::EnergyTermType dataFn(int p, int l, void *data)
{
...
void GridGraph_DArraySArray(int width,int height,int num_pixels,int num_labels)
{
	...
	GCoptimization::EnergyTermType *data = new GCoptimization::EnergyTermType[num_pixels*num_labels];
	for ( int i = 0; i < num_pixels; i++ )
		for (int l = 0; l < num_labels; l++ )
			if (i < 25 ){
	...
	GCoptimization::EnergyTermType *smooth = new GCoptimization::EnergyTermType[num_labels*num_labels];
	for ( int l1 = 0; l1 < num_labels; l1++ )
		for (int l2 = 0; l2 < num_labels; l2++ )
...
void GridGraph_DArraySArraySpatVarying(int width,int height,int num_pixels,int num_labels)
{
	...
	GCoptimization::EnergyTermType *V = new GCoptimization::EnergyTermType[num_pixels];
	GCoptimization::EnergyTermType *H = new GCoptimization::EnergyTermType[num_pixels];
...
int main(int argc, char **argv)
{
	...
	printf("\n  Finished %lld (%lld) clock per sec %lld\n", (long long int)clock()/CLOCKS_PER_SEC,
    (long long int)clock(), (long long int)CLOCKS_PER_SEC);
