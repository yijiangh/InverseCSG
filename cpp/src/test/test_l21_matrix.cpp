...
// rel_threshold: 0.1
// abs_threshold: 1e-3
void TestL21Matrix(const std::string& conf_file,
  const std::string& root, const std::string& scene) {
  if (!common::HasExtension(conf_file, ".conf")) {
    std::ostringstream ss;
    ss << "test::TestL21Matrix: incorrect conf file name: " << conf_file;
    common::PrintError(ss.str());
  }
  std::cout << "test::TestL21Matrix:" << std::endl;
  // Find example folder.
  ...
  for (int j = 0; j < faces_size; ++j) {
    if (j % 1000 == 0) {
      std::cout << "test::TestL21Matrix: testing " << j << "/" << face_num
        << " faces..." << std::endl;
    }
    const int i = faces[j];
    ...
    const double target = quad_params.transpose() * wrapper.l21_matrix(i)
      * quad_params;
    // Now estimate the integral.
    double estimate = 0.0;
    for (int j = 0; j < sample_num; ++j) {
      const double dfx = quad.Gradient(points.col(j)).squaredNorm();
      estimate += dfx;
    }
    ...
    if (std::fabs(estimate - target) >=
      ...
      std::cerr << common::RedHead()
        << "test::TestL21Matrix: integral differs too much: target = "
  ...
  std::cout << common::GreenHead() << "test::TestL21Matrix: OK."
